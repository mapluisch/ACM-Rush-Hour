...
public class Main : MonoBehaviour {
    ...
    public void CreateInitialNode () {
        Node n = new Node (null, parkingLot, null, 0);
        solutionQueue.Enqueue (n);
    }
    public List<Node> solutionPath = new List<Node> ();
    public void CreateSolutionPath (Node node) {
        if (node.parent != null) {
            solutionPath.Add (node);
            CreateSolutionPath (node.parent);
        } else {
            ...
            for (int i = solutionPath.Count - 1; i >= 0; i--) {
                print ("Car " + (solutionPath[i].movedCar.id - 1) + "  " + (solutionPath[i].movedDirection == Direction.forward ? "F" : "B"));
            }
    ...
    public void QueuePossibleMoves (Node n) {
        foreach (Car car in n.parkingLot.cars) {
            if (n.parkingLot.CanCarMove (car, Direction.forward)) {
                Node m = new Node (n, n.parkingLot, car, Direction.forward);
            ...
            if (n.parkingLot.CanCarMove (car, Direction.backward)) {
                Node m = new Node (n, n.parkingLot, car, Direction.backward);
